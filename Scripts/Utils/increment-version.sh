...
acquire_lock_dir() {
  local target_file="$1"
  local lock_dir="${target_file}.increment-version.lock"
  local waited=0
  local max_wait_seconds=30

  while ! mkdir "$lock_dir" 2> /dev/null; do
    if [[ $waited -ge $max_wait_seconds ]]; then
      echo -e "${RED}Error: Could not acquire lock '$lock_dir' after ${max_wait_seconds}s.${NC}" >&2
      return 1
    fi
...
main() {
  ...
  if ! original_version=$(
    "$PYTHON_CMD" - "$package_json_path" << 'PY'
import json
import sys
...
PY
  ...
  if [[ "$WHATIF" == "true" ]]; then
    ...
    if "$PYTHON_CMD" - "$package_json_path" "$new_version" << 'PY'; then
import os
import re
import sys
