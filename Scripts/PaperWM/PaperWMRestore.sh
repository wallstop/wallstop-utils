...
if [[ "$RESTORE_DCONF" == true ]]; then
  if [[ -f "$BACKUP_DIR/$DCONF_BACKUP_FILE" ]]; then
    ...
    dconf_dump_error_file="$(mktemp)"
    if CURRENT_SETTINGS=$(dconf dump "$DCONF_PATH" 2> "$dconf_dump_error_file"); then
      :
    else
      CURRENT_SETTINGS=""
      dump_error="$(cat "$dconf_dump_error_file")"
      if [[ -n "$dump_error" ]]; then
        echo "Warning: Could not backup existing dconf settings: $dump_error" >&2
      fi
    fi
    rm -f "$dconf_dump_error_file"
