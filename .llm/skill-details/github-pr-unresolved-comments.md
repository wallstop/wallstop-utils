...
- Normalize any user/env-derived `Cookie` or bearer-token value at the final HTTP header construction boundary: trim leading/trailing whitespace, strip CR/LF, and keep behavioral plus policy coverage so alternate call paths cannot bypass sanitization.
