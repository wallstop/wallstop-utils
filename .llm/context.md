...
18. Treat multiline PowerShell `-f` formatting as a binding-risk category: never split `-f` onto one line and a comma-continued first argument onto the next line (for example `"..." -f` newline `$arg,`). Safe examples: `"{0} {1}" -f $a, $b` and `"{0} {1}" -f @($a, $b)`. Risky example: `"{0} {1}" -f` newline `$a,` newline `$b`. `Scripts/Utils/Common/FormatOperatorSafetyHelpers.ps1` is the authoritative static guard for this continuation pattern, and `Run-PreCommitValidation.ps1` plus `Invoke-FullValidation.ps1 -PreflightOnly` must enforce it before module-dependent checks.
