...
- Child PowerShell executable resolution must use `Resolve-PowerShellExecutablePath` when either `pwsh` or Windows PowerShell fallback is acceptable; reusable executable liveness/sentinel probes must go through `Test-PowerShellExecutableCandidateReliably` instead of ad-hoc one-off probes.
