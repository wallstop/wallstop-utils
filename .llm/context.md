...
19. Git hook wrappers (`.githooks/pre-commit`, `.githooks/pre-push`) and devcontainer bootstrap preflight (`.devcontainer/post-create.sh`) must enforce bounded execution via timeout-capable wrappers with stable diagnostics (`E_HOOK_TIMEOUT`, `E_HOOK_TIMEOUT_CONFIG`) and environment-variable overrides (`WALLSTOP_PRECOMMIT_TIMEOUT_SECONDS`, `WALLSTOP_PREPUSH_TIMEOUT_SECONDS`, `WALLSTOP_DEVCONTAINER_PREFLIGHT_TIMEOUT_SECONDS`) so stalled validation commands cannot hard-lock editor-hosted workflows.
