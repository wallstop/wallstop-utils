...
4. Encoding: when writing machine-readable artifacts (for example JSON), use explicit UTF-8 no-BOM via `[System.Text.UTF8Encoding]::new($false)` and `[System.IO.File]::WriteAllText(...)`. When such an artifact is committed AND covered by a formatter hook (for example `Config/scoopfile.json` under `pretty-format-json`), write the canonical hook format (2-space, LF) at the source, because unattended backups commit with `git commit --no-verify` and bypass the hook; two formats of the same data cause recurring whole-file merge conflicts. Canonicalize with string-preserving serialization (`System.Text.Json` `JsonDocument`, runtime-gated for 5.1) -- never `ConvertFrom-Json`/`ConvertTo-Json`, which reparse ISO timestamps into `[datetime]` and timezone-shift them, corrupting the data (`Scripts/Scoop/ScoopBackup.ps1` `ConvertTo-CanonicalScoopExportJson`).
