...
19. Git hook wrappers (`.githooks/pre-commit`, `.githooks/pre-push`) and devcontainer bootstrap preflight/prewarm (`.devcontainer/post-create.sh`) must enforce bounded execution through `Scripts/Utils/Common/HookTimeout.sh` with stable diagnostics (`E_HOOK_TIMEOUT`, `E_HOOK_TIMEOUT_CONFIG`, `W_HOOK_RUNTIME_BUDGET`) and environment-variable overrides (`WALLSTOP_PRECOMMIT_TIMEOUT_SECONDS`, `WALLSTOP_PREPUSH_TIMEOUT_SECONDS`, `WALLSTOP_DEVCONTAINER_PREFLIGHT_TIMEOUT_SECONDS`, `WALLSTOP_DEVCONTAINER_PRECOMMIT_PREWARM_TIMEOUT_SECONDS`) so stalled validation commands cannot hard-lock editor-hosted workflows; `timeout`/`gtimeout` paths must use kill-after cleanup (`-k`) and avoid `--foreground`, shell fallback watchdogs must terminate an isolated process group/session (not only the direct child) and clean up lingering descendants after the wrapper exits, and Codex npm bootstrap must resolve npm-managed candidates directly, exclude `~/.local/bin/codex` from PATH fallbacks, accept local-prefix Codex only when npm package metadata proves ownership, and verify link postconditions before reporting success. The OpenCode npm bootstrap (`opencode-ai@latest`, binary `opencode`) must mirror the Codex contract, `.devcontainer/post-start.sh` (wired via `postStartCommand`) must self-heal root-owned persistent cache mounts (`/home/vscode/.npm`, `/home/vscode/.cache` — Docker provisions volume mount points root-owned when absent from the image, causing npm/pip/opencode `EACCES`) with post-create running the same repair before any npm usage, and the single sanctioned `features` entry is `ghcr.io/devcontainers/features/node:1` with `version: latest` (any other feature entry violates the image-first policy tests). Default local hooks must stay last-resort and fast: use targeted staged/pushed pathspec discovery plus streamed `git cat-file blob ... | bash -n` for shell entrypoints, do not run workflow actionlint/ShellCheck by default, and keep workflow actionlint as explicit opt-in (`WALLSTOP_PRECOMMIT_FAST_WORKFLOW_LINT=1` / `WALLSTOP_PREPUSH_FAST_WORKFLOW_LINT=1`). `Invoke-FullValidation.ps1 -PreflightOnly` only proves repo-managed native assets and hook resolver availability; agentic edits to `.github/workflows/*.yml` or `.yaml` must also run `pwsh -NoLogo -NoProfile -File Scripts/Utils/Quality/Invoke-NativeQualityChecks.ps1 -Tool actionlint -TargetFiles <workflow paths>` before relying on hooks.
...
21. GitHub GraphQL utilities must enforce case-sensitive variable payload alignment with declared operation variables (for example `$owner`/`$repo` requires `owner`/`repo` keys) before network calls, and keep both behavioral coverage in `Tests/GitHub/Get-UnresolvedPRComments.Tests.ps1` plus policy coverage in `Tests/Utils/ScriptSafetyConventions.Tests.ps1`. The unresolved-comments utility must also preserve default bot markup cleanup and Cursor/Bugbot embedded-location handling with the same behavioral plus policy coverage. Rendered output must be verbatim and copy-safe: clipboard copies of native tools (`pbcopy`/`xclip`/`xsel`/`wl-copy`) must run through `Invoke-NativeClipboardTool`, which detaches the child's standard streams (`UseShellExecute=$false`, `RedirectStandardInput/Output/Error=$true`) so a forked selection-server child cannot hold the terminal open (the "clipboard hangs the terminal for several seconds" bug), writes the payload as raw UTF-8 (no-BOM) bytes to the child stdin via a timeout-bounded `WriteAsync`, and bounds the call with `WaitForExit(timeout)` plus `Stop-ProcessTreePortably` tree-kill on overrun; `Invoke-Main` ensures UTF-8 terminal rendering via `Initialize-Utf8ConsoleOutputEncoding`, which reads `[System.Console]::OutputEncoding` first and only changes it when not already UTF-8 (code page 65001) so it never pays the slow Windows `SetConsoleOutputCP` code-page switch per invocation; the OSC52 clipboard bridge must emit an explicit-selector, UTF-8 base64 sequence via `ConvertTo-Osc52Sequence` (`ESC ] 52 ; c ; <base64> BEL`, never the ambiguous empty-selector `Set-Clipboard -AsOSC52`) with a size-budget warning (`W_CLIPBOARD_OSC52_TRUNCATION_RISK`, override `WALLSTOP_CLIPBOARD_OSC52_MAX_BYTES`) and Windows-first clipboard priority; `-Truncate` must not split a UTF-16 surrogate pair at the boundary; the text renderer must use a single collapsed `---` delimiter between blocks (no doubled seam); GitHub/Copilot/Cursor `suggestion` code fences must be extracted verbatim into a `suggestions` record field via a single shared fence regex (reused by prose stripping) and rendered under a `Suggested change:` label, except under `-KeepMarkup`; GitHub web-exposed Copilot `automatedComment.suggestion.diffEntries` may be attached best-effort by comment `databaseId` (for private web HTML only via explicit `-GitHubWebCookie` / `WALLSTOP_GITHUB_WEB_COOKIE` / `GITHUB_WEB_COOKIE`), but only `DELETION`/`ADDITION` lines may become public `suggestedDiffs`, with unified-diff `\ No newline at end of file` sentinel rows filtered as metadata before public changed-line output; and GitHub review-comment `diffHunk` / REST `diff_hunk` must stay internal to suggestion semantics: never serialize it to public JSON and never treat or render it as `Suggested change:`, but it may be rendered as trimmed `Diff context:` in TEXT output when `includeDiffHunks`/`-IncludeDiffHunks` is enabled and no higher-confidence suggestion or web diff is present; however, a `suggestion` anchored to a resolvable line range is reconstructed for TEXT output into a `-`/`+` unified diff whose before-side is the current new-side lines the suggestion replaces, recovered END-ANCHORED from `diff_hunk` (the last N new-side lines, with N the anchored span clamped to availability) by `Get-NewSideLinesInRange` + `ConvertTo-ReconstructedSuggestionDiff` (and the parallel TS `extractNewSideLinesInRange` + `reconstructSuggestionDiff` in `Extensions/WallstopPrComments/src/suggestionDiff.ts`), with the same no-newline sentinel filtered as metadata; end-anchoring is mandatory because the hunk header's absolute line number is unreliable (GitHub truncates long hunks to their tail while keeping the original `@@` header, and the thread line drifts past the hunk window after later commits, both of which previously dropped the removed lines silently), the reconstruction must never fabricate deletions when no `diff_hunk`/anchor/new-side line is available (it falls back to the verbatim suggestion), and the reconstructed diff is TEXT-only enrichment so the public JSON `suggestedChanges[].value` stays the verbatim suggestion `code`; covered behaviorally in `Tests/GitHub/Get-UnresolvedPRComments.Tests.ps1` and `Extensions/WallstopPrComments/test/suggestionDiff.test.ts` + `renderer.test.ts`. Any user/env-derived value placed in an HTTP request header (the `-GitHubWebCookie` web Cookie and the bearer token) must pass through `ConvertTo-SafeHttpHeaderValue` at the header-construction boundary (strip the full C0 control range plus DEL, which includes the CR/LF response-splitting vector, then trim, returning `$null` when nothing survives); this is a whole-file invariant, enforced by a `ScriptSafetyConventions.Tests.ps1` AST guard that requires every dynamic request-header value -- any index or member assignment to a `*headers` map, any sensitive-header (`Authorization`/`Cookie`/`Proxy-Authorization`/`Set-Cookie`) key anywhere, or a dictionary-style `.Add(<sensitive header>, value)` call -- to be a literal or a `$safe*`/sanitizer-output value, not only the functions that build headers today. A single shared predicate `Test-ThreadCommentHasRenderableContent` (renderable = non-`(none)` prose, `suggestedChanges`, or attached `suggestedDiffs`) decides `comments[]` inclusion at BOTH record creation and render time so they never drift; `diffHunk` and `suggestedDiffsUnavailableReason` must never drive inclusion (a `diffHunk`-only comment would emit an empty block and suppress the `topLevelComment`/`suggestions` fallback), except that record creation also keeps a `suggestedDiffsUnavailableReason` comment as a web-enrichment placeholder. Public text and JSON output should contain only file, line range, suggestion prose, and actual exposed suggested changes, plus optional TEXT-only `Diff context:` when requested; JSON records use lower-camel `path`, `lineStart`, `lineEnd`, and `comments[]` entries with `suggestion` plus normalized `suggestedChanges[]` (`kind`, `value`), omitting author, URL, latest-reply, and context-hunk fields. The fast-exit behavior is on by default with a `-NoFastExit` opt-out: after rendering it may skip the slow .NET/PowerShell managed teardown by flushing the console then terminating immediately (`Invoke-FastProcessExit` -> `Stop-CurrentProcessImmediately`: runtime-gated libc `_exit` on non-Windows, `[System.Environment]::Exit` on Windows/fallback); it must flush before terminating, expose `-NoFastExit` to restore standard teardown, and be wired only inside the existing dot-source run guard so dot-sourced tests never terminate the host. The run-guard fast exit is additionally gated by `Test-ShouldUseFastExit`, which returns false (forcing the safe managed exit that restores terminal state) whenever this run read from the console (`$script:TerminalInputInitialized`) or runs inside an interactive host (`Test-IsInteractiveHostSession`: PSReadLine loaded, `-NoExit`, or a pure REPL command line), because `_exit`/`[System.Environment]::Exit` skip the .NET/host terminal restoration and otherwise leave the parent shell with echo/line-editing disabled. Interactive prompts must never use `Read-Host` (it leaves a non-interactive host's terminal in raw mode); they go through `Read-TerminalResponse`, which prompts on stderr, reads canonically via `[Console]::In.ReadLine()`, and flags `TerminalInputInitialized`. `Invoke-Main` also sets `$ProgressPreference = "SilentlyContinue"` run-wide so the web-cmdlet progress bar's DSR cursor probes (`ESC[6n`) cannot pollute the shell's stdin. Source-aware auth recovery is mandatory: environment token precedence is `GH_TOKEN` before `GITHUB_TOKEN`; rejected token values from recoverable auth failures must be excluded from later resolution in the same run; prompted login fallback must bypass environment tokens and rejected values to prevent long-running invalid-token loops; direct owner/repo mode remains non-prompting: explicit `-Token` auth failures fail fast, while missing auth or recoverable ambient-token failures may use non-interactive stored-credential resolution/retry and public REST fallback before failing. Agentic edits to `Scripts/Utils/GitHub/Get-UnresolvedPRComments.ps1` must run targeted `Run-PreCommitValidation.ps1 -TargetFiles`, `Tests/GitHub/Get-UnresolvedPRComments.Tests.ps1`, and `Tests/Utils/ScriptSafetyConventions.Tests.ps1` validation before hooks.
...
29. CI-invoked PowerShell entrypoints: GitHub Actions wraps every `shell: powershell|pwsh` run block as `$ErrorActionPreference = 'stop'` + block + `if ((Test-Path -LiteralPath variable:\LASTEXITCODE)) { exit $LASTEXITCODE }`, so step exit = the last native command's `$LASTEXITCODE` whenever any ran (even after later success output), else final success state; therefore `Scripts/**` entrypoints newly wired directly into `.github/workflows/*.yml` steps (precedent: `Scripts/Utils/Quality/Install-PowerShellQualityModules.ps1`) must end their `-NoInvokeMain`/dot-source-guarded main path with an explicit `exit 0` (never top-level, so library/test dot-sourcing cannot exit the host) and signal failures only via terminating errors/non-zero exits — handled-degradation success paths following nonzero-capable native invocations are the exact failure class this prevents. Pinned in `Tests/Utils/Install-PowerShellQualityModules.Tests.ps1` ("CI dot-source exit-code contract"). Existing workflow-invoked scripts without a trailing success exit are normalized opportunistically when next touched.
...
- `ProcessStartInfo` argument and environment mutations must use `Set-PortableProcessArguments` / `Set-PortableProcessEnvironmentVariable`; PATH-shim harnesses must preflight `command -v` for fake commands. `.ps1` shims faking external CLIs must emit payload through the pipeline (`Write-Output`; `[Console]::Out.Write` bypasses the pipeline, so `@(& shim 2>&1)` captures nothing, while native-command stdout is auto-wrapped into it), escape embedded quotes per target grammar (bash close/escape/reopen vs PowerShell doubling), and line-attributing parsers of captured external output should expand elements with `-split "\`r?\`n|\`r"` so lone-CR progress-redraw frames also split (otherwise `^`/`$`-anchored attribution regexes silently never match), because script-shim captures arrive as single multi-line elements while native captures arrive per-line. Pairing captured progress lines to discovered records (e.g., winget Found-block parsers) must treat a discovery-line reprint as a continuation of an open block only when position AND id match, fail closed (Unresolved/UnownedFailure, never false-green) on different-id interleaves and post-terminal stale redraws, and derive status solely from explicit terminal markers — precedent Resolve-WinGetUpdateOutcome + Tests/WinGet/WinGetUpdate.Tests.ps1.
- Child PowerShell executable resolution must use `Resolve-PowerShellExecutablePath` when either `pwsh` or Windows PowerShell fallback is acceptable; reusable executable liveness/sentinel probes must go through `Test-PowerShellExecutableCandidateReliably` instead of ad-hoc one-off probes. Capability probes that invoke native interpreters (for example `python3 -c`) must be gated to the platform where their feature actually runs: Windows PowerShell 5.1 turns redirected native stderr (`2>$null`/`2>&1`) into terminating errors under `$ErrorActionPreference = 'Stop'`, so a probe inside shared BeforeAll code fails whole Describes instead of skipping; prefer capturing merged streams into an array plus exit-code checks on hosts that do run the probe.
- `[ArgumentCompletions()]`, ternary `?:`, `??`/`??=`, `&&`/`||`, `clean{}`, `$PSStyle` are 7+-only;
  ...
  options (`-PredictionSource`/`-PredictionViewStyle`) behind a capability probe; backup flows must
  self-heal drifted profiles through the shared `Restore-PowerShellProfileFromValidatedSource`
  helper (validated repo source, timestamped backup, rollback-on-invalid-result) and fail closed
  surfacing the heal-failure reason instead of shipping a drifted profile.
...
4. Encoding: when writing machine-readable artifacts (for example JSON), use explicit UTF-8 no-BOM via `[System.Text.UTF8Encoding]::new($false)` and `[System.IO.File]::WriteAllText(...)`. When such an artifact is committed AND covered by a formatter hook (`Config/scoopfile.json`, `Config/WindowsTerminal/settings.json`, `Config/Komorebi/profiles/*/*.json` under `pretty-format-json`), write the canonical hook format at the source, because unattended backups commit with `git commit --no-verify` and bypass the hook; two formats of the same data cause recurring whole-file merge conflicts. Every such writer (`Scripts/Scoop/ScoopBackup.ps1`, `Scripts/WindowsTerminal/WindowsTerminalBackup.ps1`, `Scripts/Komorebi/KomorebiProfileHelpers.ps1`) must route through the single shared canonicalizer `Scripts/Utils/Common/CanonicalJsonHelpers.ps1` (`ConvertTo-CanonicalJsonText` / `Write-CanonicalJsonFile`) -- do not reimplement per writer. It uses string-preserving `System.Text.Json` `JsonDocument` (runtime-gated for 5.1) so ISO timestamps survive verbatim -- never `ConvertFrom-Json`/`ConvertTo-Json` to re-emit, which reparse timestamps into `[datetime]` and timezone-shift them. To be byte-identical to `pretty-format-json` (Python `json.dumps`) it (a) normalizes `WriteIndented` output to LF + one trailing newline because `Utf8JsonWriter` emits CRLF on Windows/.NET (<9), and (b) escapes non-ASCII to lowercase `\uXXXX` (`ensure_ascii=True`) via `ConvertTo-AsciiEscapedJsonText` on top of `UnsafeRelaxedJsonEscaping` (no single built-in .NET encoder matches; `Default`/`Create` over-escape `<>&'+`). `Tests/Utils/CanonicalJsonHelpers.Tests.ps1` asserts each committed artifact is a fixed point of the canonicalizer, catching any drift on every platform.
...
16. `Backup.ps1` must classify the worktree before any backup step executes: pre-existing changes outside managed `Config/` must fail with `E_BACKUP_GIT_TREE_DIRTY_PREFLIGHT`, while managed-only drift may proceed through a bounded stash/pull/restore cycle with explicit `W_BACKUP_GIT_MANAGED_DIRTY_PREFLIGHT` and stash/restore failure diagnostics.
