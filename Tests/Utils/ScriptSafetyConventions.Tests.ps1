...
Describe "Backup script safety conventions" {
    It "tracks per-step results and emits best-effort summary in Backup orchestrator" {
        ...
        $backupScript | Should -Match 'W_BACKUP_GIT_MANAGED_DIRTY_PREFLIGHT'
        $backupScript | Should -Match 'function\s+Invoke-BackupGitPullWithManagedChanges'
        $backupScript | Should -Match 'E_BACKUP_GIT_MANAGED_STASH_FAILED'
        $backupScript | Should -Match 'E_BACKUP_GIT_MANAGED_STASH_REFERENCE_FAILED'
        $backupScript | Should -Match 'E_BACKUP_GIT_MANAGED_RESTORE_FAILED'
        ...
        $backupScript | Should -Match 'Assert-BackupGitTreeCleanPreflight\s+-GitExecutable\s+\$gitExecutable\s+-RepositoryRoot\s+\$repositoryRoot\s+-ManagedPathspecs\s+\$managedPathspecs[\s\S]*?Assert-BackupGitBranchOrThrow\s+-GitExecutable\s+\$gitExecutable\s+-RepositoryRoot\s+\$repositoryRoot\s+-ExpectedBranch\s+"main"[\s\S]*?Invoke-BackupGitPullWithManagedChanges\s+-GitExecutable\s+\$gitExecutable\s+-RepositoryRoot\s+\$repositoryRoot\s+-ManagedPathspecs\s+\$managedPathspecs[\s\S]*?foreach\s*\(\$step\s+in\s+\$applicableSteps\)'
        ...
        $backupScript | Should -Match '\$outsideManagedPathspec\s*=\s*@\("\."\)[\s\S]*?\$outsideManagedPathspec\s+\+=\s+":\(exclude\)\$managedPathspec"'
        $backupScript | Should -Match '"stash",\s+"push",\s+"--include-untracked"'
        $backupScript | Should -Match 'stash\s+pop\s+--index'
    ...
    It "keeps Update orchestrator rooted at script directory without nested Scripts suffix" {
        ...
        $updateScript | Should -Match 'function\s+Resolve-UpdateElevationStartFailure'
        $updateScript | Should -Match 'E_UPDATE_ELEVATION_START_FAILED'
        $updateScript | Should -Match 'W_UPDATE_DOT_SOURCE_NO_OP'
