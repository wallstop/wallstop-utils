...
Describe "CI scope expansion" {
    ...
    It "routes cross-version PowerShell workflow tests through the shared Pester gate" {
        ...
        $workflow | Should -Match 'Run Pester suite under Windows PowerShell 5\.1[\s\S]*\$winPs51TestPaths\s*=\s*@\([\s\S]*WindowsPowerShell51RuntimeSmoke\.Tests\.ps1[\s\S]*CompatibilityHelpers\.Tests\.ps1[\s\S]*CompatibilityConventions\.Tests\.ps1[\s\S]*Install-PowerShellQualityModules\.Tests\.ps1[\s\S]*Get-UnresolvedPRComments\.Tests\.ps1'
        $workflow | Should -Match 'Run Pester suite under Windows PowerShell 5\.1[\s\S]*Invoke-PesterQualityGate\.ps1[\s\S]*-TestPath\s+\$winPs51TestPaths[\s\S]*-OutputVerbosity\s+None[\s\S]*-TestResultOutputPath\s+"testresults-winps51\.xml"'
...
Describe "Cross-language quality platform conventions" {
    It "defines a pinned pre-commit configuration with required hook coverage" {
        ...
        $powershellPreCommitHookBlock | Should -Match ([regex]::Escape('precommit-cli-tools'))
    ...
    It "pins and verifies the pre-commit CLI itself" {
        ...
        $cliHelper | Should -Match 'precommit-cli-tools\.json'
        $cliHelper | Should -Match 'Resolve-PreCommitCliUvExecutable'
    ...
    It "keeps git hooks last-resort with fallback guidance" {
        ...
        $hookFastToolResolver = Get-Content -Path (Join-Path -Path $script:repoRoot -ChildPath 'Scripts/Utils/Common/HookFastToolResolver.sh') -Raw
        ...
        $preCommitHook | Should -Match 'repo_root_stderr_path="\$\(mktemp\s+2>\s*/dev/null\)"'
        ...
        $preCommitHook | Should -Match 'repo_root_stderr="\$\(<\s*"\$repo_root_stderr_path"\)"'
        ...
        $preCommitHook | Should -Match 'get_staged_precommit_files'
        $preCommitHook | Should -Match 'git -C "\$repo_root" diff --cached --name-only --diff-filter=ACMRD -- 2> /dev/null'
        $preCommitHook | Should -Match 'get_staged_precommit_fast_files'
        $preCommitHook | Should -Match 'git -C "\$repo_root" diff --cached --name-only --diff-filter=ACMR --'
        $preCommitHook | Should -Match '"Scripts/\*\.sh"'
        $preCommitHook | Should -Match '"\.github/workflows/\*\.yml"'
        $preCommitHook | Should -Match '"\.github/workflows/\*\.yaml"'
        $preCommitHook | Should -Match 'run_fast_precommit_gate'
        $preCommitHook | Should -Match 'WALLSTOP_PRECOMMIT_FULL_SUITE'
        $preCommitHook | Should -Match 'caller_errexit_enabled'
        $preCommitHook | Should -Match 'precommit_force_recovery_suite'
        $preCommitHook | Should -Match 'W_PRECOMMIT_STAGED_DISCOVERY_FAILED'
        $preCommitHook | Should -Match 'W_PRECOMMIT_FAST_STAGED_DISCOVERY_FAILED'
        $preCommitHook.IndexOf('run_safe_autorepair', [System.StringComparison]::Ordinal) | Should -BeLessThan $preCommitHook.IndexOf('WALLSTOP_PRECOMMIT_FULL_SUITE', [System.StringComparison]::Ordinal)
        $preCommitHook | Should -Match 'staged_precommit_files="\$\(get_staged_precommit_files\)"[\s\S]*staged_precommit_fast_files="\$\(get_staged_precommit_fast_files\)"'
        $preCommitHook | Should -Match 'if \[\[ "\$precommit_force_recovery_suite" -eq 0 \]\] && ! is_truthy "\$\{WALLSTOP_PRECOMMIT_FULL_SUITE:-0\}"; then[\s\S]*run_fast_precommit_gate "\$staged_precommit_fast_files"'
        $preCommitHook | Should -Match 'run_precommit_fast_diff_check'
        $preCommitHook | Should -Match 'git -C "\$repo_root" diff --cached --check --'
        $preCommitHook | Should -Match 'WALLSTOP_PRECOMMIT_FAST_WORKFLOW_LINT'
        $preCommitHook | Should -Match 'HookFastToolResolver\.sh'
        $preCommitHook | Should -Match 'wallstop_resolve_managed_fast_tool "\$repo_root" "\.tools/native-quality" "actionlint"'
        $preCommitHook | Should -Not -Match 'wallstop_resolve_managed_fast_tool "\$repo_root" "\.tools/shell-quality" "shellcheck"'
        $preCommitHook | Should -Match 'validate_precommit_shell_blob'
        $preCommitHook | Should -Match 'git -C "\$repo_root" cat-file blob ":\$\{staged_file\}"\s*\|\s*bash -n'
        $preCommitHook | Should -Match 'extract_precommit_index_blob_for_fast_validation'
        $preCommitHook | Should -Match 'git show ":\$\{staged_file\}"'
        $preCommitHook | Should -Match 'E_PRECOMMIT_FAST_BLOB_EXTRACT_FAILED'
        $preCommitHook | Should -Match '-shellcheck "" -pyflakes ""'
        $preCommitHook | Should -Not -Match 'run_fast_tool_version_probe'
        $preCommitHook | Should -Not -Match '\$tool_path"\s+--?version\s+2>&1\s+\|\|\s+true'
        $preCommitHook | Should -Match 'Invoke-NativeQualityChecks\.ps1" -Tool actionlint -EnsureOnly'
        $preCommitHook | Should -Not -Match '\[\[ -f "\$file" \]\] \|\| continue'
        ...
        $preCommitHook | Should -Match 'set\s+\+e\s*\r?\n\s*run_with_timeout "\$remaining_seconds" "pre-commit hook-stage pre-commit" pwsh[\s\S]*recovery_exit=\$\?\s*\r?\n\s*set -e'
        $preCommitHook | Should -Match 'set\s+\+e\s*\r?\n\s*run_with_timeout "\$remaining_seconds" "pre-commit hook-stage pre-commit" pre-commit run[\s\S]*precommit_hook_exit=\$\?\s*\r?\n\s*set -e'
        ...
        $preCommitHook | Should -Match 'PRE_COMMIT_HOME="\$precommit_home"'
        $preCommitHook | Should -Match '\.tools/precommit-cli/pre-commit-home'
        ...
        $prePushHook | Should -Match 'repo_root_stderr_path="\$\(mktemp\s+2>\s*/dev/null\)"'
        ...
        $prePushHook | Should -Match 'repo_root_stderr="\$\(<\s*"\$repo_root_stderr_path"\)"'
        ...
        $prePushHook | Should -Match 'run_fast_prepush_gate'
        $prePushHook | Should -Match 'WALLSTOP_PREPUSH_FULL_SUITE'
        $prePushHook | Should -Match 'WALLSTOP_PREPUSH_FAST_WORKFLOW_LINT'
        $prePushHook | Should -Match 'HookFastToolResolver\.sh'
        $prePushHook | Should -Match 'wallstop_resolve_managed_fast_tool "\$repo_root" "\.tools/native-quality" "actionlint"'
        $prePushHook | Should -Not -Match 'wallstop_resolve_managed_fast_tool "\$repo_root" "\.tools/shell-quality" "shellcheck"'
        $prePushHook | Should -Match 'saw_non_head_push_ref'
        $prePushHook | Should -Match 'git merge-base --is-ancestor "\$local_oid" HEAD'
        $prePushHook | Should -Match 'validate_prepush_shell_blob'
        $prePushHook | Should -Match 'git cat-file blob "\$\{local_oid\}:\$\{changed_file\}"\s*\|\s*bash -n'
        $prePushHook | Should -Match 'extract_prepush_blob_for_fast_validation'
        $prePushHook | Should -Match 'git cat-file -e "\$\{local_oid\}:\$\{changed_file\}"'
        $prePushHook | Should -Match 'git show "\$\{local_oid\}:\$\{changed_file\}"'
        $prePushHook | Should -Match '-shellcheck "" -pyflakes ""'
        $prePushHook | Should -Not -Match 'run_fast_tool_version_probe'
        $prePushHook | Should -Not -Match '\$tool_path"\s+--?version\s+2>&1\s+\|\|\s+true'
        $prePushHook | Should -Match 'Invoke-NativeQualityChecks\.ps1" -Tool actionlint -EnsureOnly'
        $prePushHook | Should -Match 'git ls-tree -r --name-only "\$local_oid"'
        $prePushHook | Should -Match 'git rev-parse "\$\{local_oid\}:\$\{file\}"'
        $prePushHook | Should -Match 'git hash-object -- "\$file"'
        $prePushHook | Should -Match 'git rev-parse ":\$\{file\}"'
        $prePushHook | Should -Match 'E_PREPUSH_FULL_NON_HEAD_REF'
        ...
        $prePushHook | Should -Match 'PRE_COMMIT_HOME="\$precommit_home"'
        $prePushHook | Should -Match '\.tools/precommit-cli/pre-commit-home'
        ...
        $hookFastToolResolver | Should -Match 'wallstop_resolve_managed_fast_tool'
        $hookFastToolResolver | Should -Match 'wallstop_fast_candidate_keys'
        $hookFastToolResolver | Should -Match 'wallstop_fast_is_windows_arm64_host'
        $hookFastToolResolver | Should -Match 'windows-arm64'
        $hookFastToolResolver | Should -Match 'windows-x64'
        $hookFastToolResolver | Should -Match 'wallstop_fast_read_manifest_asset_property'
        $hookFastToolResolver | Should -Match 'preferred_candidate_failed'
        $hookFastToolResolver | Should -Match 'wallstop_fast_validate_tool_candidate'
        $hookFastToolResolver | Should -Match 'wallstop_fast_compute_tool_fingerprint'
        $hookFastToolResolver | Should -Match 'expected_asset_name'
        $hookFastToolResolver | Should -Match 'expected_asset_sha256'
        $hookFastToolResolver | Should -Match 'assetName'
        $hookFastToolResolver | Should -Match 'sha256'
        $hookFastToolResolver | Should -Match 'executableSha256'
        $hookFastToolResolver | Should -Match 'executableSize'
        $hookFastToolResolver | Should -Match 'executableMtime'
        $hookFastToolResolver | Should -Match 'executableFastFingerprintVersion'
        $hookFastToolResolver | Should -Match 'executableFastFingerprint'
        $hookFastToolResolver | Should -Match 'wallstop_fast_fingerprint_version'
        $hookFastToolResolver | Should -Match 'wallstop_fast_read_file_mtime'
        $hookFastToolResolver | Should -Match 'tail -c "\$length" "\$tool_path"'
        $hookFastToolResolver | Should -Match '\$\{tool_name\}/\$\{expected_version\}/\$\{candidate_key\}/bin'
    }
    ...
    It "bootstraps pinned native tools during validation preflight" {
        ...
        $sharedHelper | Should -Match 'E_\$\(\$Context\.DiagnosticPrefix\)_PLATFORM_UNSUPPORTED'
        $sharedHelper | Should -Match 'E_\$\(\$Context\.TargetDiagnosticPrefix\)_TARGET_OUTSIDE_REPOSITORY'
        $sharedHelper | Should -Match 'executableSha256'
        $sharedHelper | Should -Match 'executableSize'
        $sharedHelper | Should -Match 'executableMtime'
        $sharedHelper | Should -Match 'executableFastFingerprintVersion'
        $sharedHelper | Should -Match 'executableFastFingerprint'
        $sharedHelper | Should -Match 'Get-QualityToolingFileSha256'
        $sharedHelper | Should -Match 'Get-QualityToolingFileModifiedUnixSeconds'
        $sharedHelper | Should -Match 'Get-QualityToolingExecutableFastFingerprint'
        $sharedHelper | Should -Match 'marker is missing executableSha256; reinstalling'
        $sharedHelper | Should -Match 'marker is missing executableFastFingerprint; reinstalling'
        $sharedHelper | Should -Match 'executable fast fingerprint does not match marker; reinstalling'
        $sharedHelper | Should -Match 'executable hash does not match marker; reinstalling'
    ...
    It "auto-recovers pre-commit hook environment corruption before falling back to manual triage" {
        ...
        $recoveryScript | Should -Match 'Get-PreCommitManagedEnvironment'
        $recoveryScript | Should -Match 'Set-PortableProcessEnvironmentVariable'
...
Describe "Backup script safety conventions" {
    ...
    It "ScoopBackup writes scoopfile.json in the canonical hook format without corrupting timestamps" {
        $fullPath = Join-Path -Path $script:repoRoot -ChildPath "Scripts/Scoop/ScoopBackup.ps1"
        $content = Get-Content -Path $fullPath -Raw
        $tokens = $null
        $parseErrors = $null
        $ast = [System.Management.Automation.Language.Parser]::ParseFile($fullPath, [ref]$tokens, [ref]$parseErrors)
        ...
        # scoopfile.json is generated by `scoop export` (4-space) but committed; an unattended backup
        # commit bypasses the pretty-format-json hook with --no-verify, so the script must itself write
        # the canonical 2-space/LF form to stop the recurring whole-file merge conflicts.
        $canonicalizer = Get-RequiredFunctionDefinitionAst -Ast $ast -Name "ConvertTo-CanonicalScoopExportJson" -Context "Scoop canonical JSON"
        ...
        # The timestamp-corrupting round-trip must NEVER be invoked on the scoop export text. An AST
        # command scan (not a text match) is used so the cautionary comment naming these cmdlets does
        # not trip the guard.
        $jsonRoundTripCalls = @($ast.FindAll({
                    param($node)
                    $node -is [System.Management.Automation.Language.CommandAst] -and
                    @("ConvertFrom-Json", "ConvertTo-Json") -contains $node.GetCommandName()
                }, $true))
        $jsonRoundTripCalls.Count | Should -Be 0 -Because "ConvertFrom-Json/ConvertTo-Json reparse ISO timestamps into [datetime] and shift their timezone, corrupting scoopfile.json"

        # The writer must route scoop's output through the canonicalizer before persisting.
        $content | Should -Match 'ConvertTo-CanonicalScoopExportJson\s+-RawJson'
        $content | Should -Match '\[System\.IO\.File\]::WriteAllText'

        # A dot-source guard keeps the backup (which needs the Windows-only scoop CLI) from running when
        # tests dot-source the script for the pure canonicalizer.
        $content | Should -Match '\$MyInvocation\.InvocationName\s+-ne\s+"\."'

        $testsPath = Join-Path -Path $script:repoRoot -ChildPath "Tests/Scoop/ScoopBackup.Tests.ps1"
        Test-Path -LiteralPath $testsPath | Should -BeTrue -Because "the canonicalizer must have behavioral coverage"
        $testsContent = Get-Content -Path $testsPath -Raw
        $testsContent | Should -Match 'preserves ISO-8601 timestamps verbatim'
        $testsContent | Should -Match 'stable fixed point'
    }
}
...
'@
    ...
    It "documents hook timeout guardrails in context and precommit skill guidance" {
        ...
        $contextContent | Should -Match 'Invoke-NativeQualityChecks\.ps1 -Tool actionlint -TargetFiles <workflow paths>'
        $skillDetailContent | Should -Match 'Invoke-NativeQualityChecks\.ps1 -Tool actionlint -TargetFiles <workflow paths>'
        ...
        $skillDetailContent | Should -Match 'asset name/asset SHA256'
        $skillDetailContent | Should -Match 'x64 fallback only when the manifest lacks a Windows ARM64 asset'
